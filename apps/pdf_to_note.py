...
def extract_images_from_pdf(pdf_path: Path, options: PdfOptions) -> list[ImageData]:
    ...
    """
    console.print(f"[yellow]Extracting images from PDF:[/yellow] {pdf_path}")
...
def transcribe_images_to_markdown(images: list[ImageData]) -> TranscriptionResponse:
    ...
    console.print(f"[yellow]Transcribing {len(images)} images using vision model...[/yellow]")
...
def extract_pdf_images(
    ...
    Example: extract_pdf_images(Path("notes.pdf"))
    """
    if not pdf_path.exists():
        raise FileNotFoundError(f"PDF file not found: {pdf_path}")
    
    console.print(f"[bold blue]Step 1: Extracting images from PDF:[/bold blue] {pdf_path}")
    ...
    images = extract_images_from_pdf(pdf_path, options)
...
def images_to_markdown(
    ...
    """
    console.print(f"[bold blue]Step 2: Converting {len(images)} images to markdown[/bold blue]")
    ...
    preview = transcription.content[:200] + "..." if len(transcription.content) > 200 else transcription.content
...
def markdown_to_note(
    ...
    note_path = note_add(NoteAddArgs(
...
def pdf_to_note(
    ...
    Example: pdf_to_note(Path("notes.pdf"), title="Lecture Notes", tags=["physics", "equations"])
    """
    console.print(f"[bold blue]Converting PDF to note:[/bold blue] {pdf_path}")
    
    # Step 1: Extract images
    images = extract_pdf_images(pdf_path, image_width, image_height, jpeg_quality)
    
    # Step 2: Convert to markdown
    transcription = images_to_markdown(images)
    
    # Step 3: Create note
    note_path = markdown_to_note(transcription, title, tags)
